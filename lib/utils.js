 ...
 * @param {array} names
 */
var AsyncDataList = function (models, names) {
  if (!(this instanceof AsyncDataList))
    return new AsyncDataList(models, names);
  this.models = models;
  this.names = names;
}
 ...
 */
AsyncDataList.prototype.getItem = function (name, data, models, callback) {
  var self = this;
  try {
    ...
    if (typeof model === 'undefined') {
      ...
      // 当实在找不到时才抛出异常
      if (typeof model === 'undefined')
        throw Error('Not register model "' + name + '"');
    }
    
    // 如果为function，则调用其以取得数据
    if (typeof model === 'function') {
      model(function (err, d) {
        if (err)
          return callback(err);
        self.saveItem(name, d, data);
        return callback(null);
      });
    }
    else {
      self.saveItem(name, model, data);
      return callback(null);
    }
 ...
 * @param {object} data
 */
AsyncDataList.prototype.saveItem = function (name, value, data) {
  var ns = name.split('.');
  var oldns = [];
  var key = ns.pop();
...
AsyncDataList.prototype.start = function (callback) {
  ...
  try {
    var next = function (err) {
      ...
      self.getItem(name, data, models, next);
    }
    next();
  }
  catch (err) {
...
AsyncDataList.prototype.startParallel = function (callback) {
  ...
  var getItem = function (name, data, models, names) {
    self.getItem(name, data, models, function (err) {
      if (err && !isReturn) {
        isReturn = true;
        return callback(err);
      }
  ...
  try {
    for (var i in names) {
      getItem(names[i], data, models, names);
    }
  }
  catch (err) {
    return callback(err);
