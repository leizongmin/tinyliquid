  ...
  if (text.length > 0) {
    if (utils.isQuoteWrapString(text)) {
      ...
      if (text.substr(0, 8) === 'forloop.') {
        // forloop变量
        return utils.astNode(OPCODE.FORLOOPLOCALS, text.substr(8).toLowerCase());
      } else if (text.substr(0, 13) === 'tablerowloop.') {
        // tablerowloop变量
        return utils.astNode(OPCODE.TABLEROWLOOPLOCALS, text.substr(13).toLowerCase());
      } else {
        if (context && context.forItems.test(text)) {
          // forw循环内的item变量
          return utils.astNode(OPCODE.FORLOOPITEM, text);
        } else if (context && context.tablerowItems.test(text)) {
          // tablerow循环内的item变量
          return utils.astNode(OPCODE.TABLEROWITEM, text);
        } else {
 ...
 */
utils.asyncEach = function (list, fn, callback, a1, a2, a3) {
  var i = -1;
  var j = 0;
  var len = list.length;
  var next = function () {
 ...
 * @param {Function} callback
 */
utils.asyncFor = function (test, fn, callback) {
  var next = function () {
    if (test()) {
      fn(next);
    } else {
