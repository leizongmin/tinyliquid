    ...
    if (utils.isQuoteWrapString(text)) {
      ...
      if (text.substr(0, 8) === 'forloop.') {
        ...
        if (context && context.forItems.test(text)) {
          // forw循环内的item变量
          return context.astNode(OPCODE.FORLOOPITEM, text);
        } else if (context && context.tablerowItems.test(text)) {
          // tablerow循环内的item变量
          return context.astNode(OPCODE.TABLEROWITEM, text);
        } else {
          // 变量名
          var childs = text.split('.');
          return context.astNode(OPCODE.LOCALS, text, childs[0], childs.length > 1 ? childs.slice(1) : null);
        }
      }
    }
  } else {
