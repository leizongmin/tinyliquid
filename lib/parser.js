...
var parser = exports = module.exports = function (tpl, options) {
  ...
  var flush = function () {
    if (strTmp.length > 0) {
      astStack.push(astNode(OPCODE.PRINTSTRING, strTmp));
    }
    strTmp = '';
  };
...
var baseTags = {
  ...
  'endif': function (context, name, body) {
    ...
    var reset = function (ast) {
      if (ast.length > 4) {
        var a = ast.slice(0, 3);
        a[3] = reset(astNode.apply(null, [OPCODE.IF].concat(ast.slice(3))));
        return a;
        return 
      } else {
        return ast;
  ...
  'for': function (context, name, body) {
    ...
    var attrs = arr[2];
    context.astStack.newChild(astNode(OPCODE.FOR, localsAstNode(arr[0], context), arr[1],
  ...
  'tablerow': function (context, name, body) {
    ...
    context.astStack.newChild(astNode(OPCODE.TABLEROW, localsAstNode(arr[0], context), arr[1],
  ...
  'endraw': function (context, name, body) {
    if (name.toLowerCase() === 'endraw') {
      context.astStack.push(astNode(OPCODE.PRINTSTRING, context.raw));
      return true;
    } else {
      return false;
    }
...
var parseCondition = parser.parseCondition = function (body, context) {
  ...
  blocks.forEach(function (item) {
    if (Array.isArray(item)) {
      if (item.length === 1) {
        var ast = astNode(OPCODE.EXISTS, localsAstNode(item[0], context));
      } else if (item.length === 2) {
        var code = OPCODE[item[0].toUpperCase()] || OPCODE.DEBUG;
        var ast = astNode(code, localsAstNode(item[1], context));
      } else {
        var code = OPCODE[item[1].toUpperCase()] || OPCODE.DEBUG;
        var ast = astNode(code, localsAstNode(item[0], context), localsAstNode(item[2], context));
      }
      condAst.push(ast);
    } else {
      condAst.push(item);
...
var parseWhen = parser.parseWhen = function (body, context) {
  ...
  blocks = blocks.filter(function (item) {
    ...
    if (!Array.isArray(ast)) ast = [OPCODE.OBJECT, ast];
    return ast;
  });
  return blocks;
};
...
var parseOutput = function (text, context) {
  ...
  if (Array.isArray(astList)) {
    if (astList[0] === OPCODE.LOCALS) {
      // 针对输出变量的优化
      return astNode(OPCODE.PRINTLOCALS, astList[1]);
    } else {
      return astNode(OPCODE.PRINT, astList);
    }
  } else {
    return astNode(OPCODE.PRINTSTRING, astList);
  }
};
