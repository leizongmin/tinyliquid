  ...
  'include': function (context, name, body) {
    ...
    if (blocks.length === 0) {
      ...
      if (blocks.length >= 3 && blocks[blocks.length - 2].toLowerCase() === 'with') {
        ...
        parameters = localsAstNode(stripQuoteWrap(blocks[blocks.length - 1]), context);
    ...
    //console.log(filename); console.log(parameters); console.log('---');
    if (parameters) {
      context.astStack.push(context.astNode(OPCODE.INCLUDE, filename, parameters));
    } else {
    ...
    if (filename.substr(0, 2) === '{{' && filename.substr(-2) === '}}') {
      filename = parseVariables(filename.slice(2, -2), context);
    } else {
