...
var parser = exports = module.exports = function (tpl, options) {
  ...
  // 编译器信息
  context.astStack.push(context.astNode(OPCODE.COMPILER, 1));
...
var baseTags = {
  ...
  'if': function (context, name, body) {
    ...
    context.astStack.newChild(context.astNode(OPCODE.IF, ast)).newChild();
  },


  'unless': function (context, name, body) {
    var ast = parseCondition(body, context);
    context.astStack.newChild(context.astNode(OPCODE.IF, context.astNode(OPCODE.NOT, ast))).newChild();
  },
  ...
  'when': function (context, name, body) {
    ...
    context.astStack.push(context.astNode(OPCODE.WHEN, ast)).newChild();
  },
...
var parseOutput = function (text, context) {
  ...
  if (Array.isArray(astList)) {
    if (astList[0] === OPCODE.LOCALS) {
      // 针对输出变量的优化
      return context.astNode(OPCODE.PRINTLOCALS, astList[1]);
    } else {
