...
var parser = exports = module.exports = function (tpl, options) {
  ...
  // 编译器信息
  context.astStack.push(astNode('$$compiler', astNode('object', {
    version:  1  // 编译器版本
  })));
  ...
  var flush = function () {
    if (strTmp.length > 0) {
      astStack.push(astNode('print', strTmp));
    }
    strTmp = '';
  };
...
var baseTags = {
  ...
  'if': function (context, name, body) {
    ...
    context.astStack.newChild(astNode('if', ast)).newChild();
  },


  'unless': function (context, name, body) {
    var ast = parseCondition(body);
    context.astStack.newChild(astNode('if', astNode('not', ast))).newChild();
  },
  ...
  'case': function (context, name, body) {
    ...
    context.astStack.newChild(astNode('case')).newChild(ast);
  },
  ...
  'when': function (context, name, body) {
    ...
    context.astStack.push(astNode('when', ast)).newChild();
  },
  ...
  'for': function (context, name, body) {
    ...
    var attrs = arr[2];
    context.astStack.newChild(astNode('for', localsAstNode(arr[0]), arr[1],
  ...
  'tablerow': function (context, name, body) {
    ...
    context.astStack.newChild(astNode('tablerow', localsAstNode(arr[0]), arr[1],
  ...
  'assign': function (context, name, body) {
    ...
    if (i !== -1) {
      ...
      context.astStack.push(astNode('assign', left, ast));
    }
  },
  ...
  'capture': function (context, name, body) {
    ...
    context.astStack.newChild(astNode('capture', name));
  },
  ...
  'cycle': function (context, name, body) {
    ...
    if (blocks.length > 0) {
      ...
      context.astStack.push(astNode.apply(null, ['cycle', key].concat(blocks)));
    }
  },
  ...
  'include': function (context, name, body) {
    ...
    if (blocks.length >= 3 && blocks[1].toLowerCase() === 'with') {
      context.astStack.push(astNode('include', stripQuoteWrap(blocks[0]), stripQuoteWrap(blocks[2])));
    } else {
      context.astStack.push(astNode('include', stripQuoteWrap(blocks[0])));
    }
  },
  ...
  'endraw': function (context, name, body) {
    if (name.toLowerCase() === 'endraw') {
      context.astStack.push(astNode('print', context.raw));
      return true;
    } else {
      return false;
    }
  ...
  'endcomment': function (context, name, body) {
    if (name.toLowerCase() === 'endcomment') {
      context.astStack.push(astNode('comment', context.raw));
      return true;
    } else {
      return false;
    }
...
var parseFilter = parser.parseFilter = function (text, firstArg, link) {
  ...
  var ast = astNode.apply(null, ['filter', name].concat(args));
  if (link.length > 0) {
...
var parseCondition = parser.parseCondition = function (body) {
  ...
  blocks.forEach(function (item) {
    if (Array.isArray(item)) {
      if (item.length === 1) {
        var ast = astNode('exists', localsAstNode(item[0]));
      } else if (item.length === 2) {
        var ast = astNode(item[0], localsAstNode(item[1]));
      } else {
        var ast = astNode(item[1], localsAstNode(item[0]), localsAstNode(item[2]));
      }
      condAst.push(ast);
    } else {
      condAst.push(item);
  ...
  var mergeCond = function (op) {
    ...
    for (var i = 0, len = _condAst.length; i < len; i++) {
      ...
      if (typeof(mid) === 'string' && mid.toLowerCase() === op && i + 2 < len) {
        condAst.push(astNode(op, _condAst[i], _condAst[i + 2]));
        i += 2;
      } else {
...
var parseTag = function (context, text) {
  ...
  if (typeof(context.tags[name]) === 'function') {
    ...
    context.astStack.push(astNode(name, body));
  }
};
