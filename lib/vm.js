...
var run = exports.run = function (astList, context, callback) {
  if (isAST(astList)) {
    if (isOpcode(astList)) {
      var op = execOpcode[astList[0]];
      if (!op) op = execOpcode[0];
      op(astList, context, callback);
    } else {
      // 执行AST节点返回的值
      var retval = [];
      asyncEach(astList, function (item, i, done) {
...
// 无法识别的操作码
execOpcode[0] = function (ast, context, callback) {
...
execOpcode[OPCODE.PRINTLOCALS] = function (ast, context, callback) {
  execOpcode[OPCODE.LOCALS]([OPCODE.LOCALS, ast[1]], context, function (err, val) {
    if (!err) context.PRINT(val);
    callback(err);
  });
};
*/
...
var getLocals = function (name, context, callback) {
  var info = context.getLocals(name);
  if (info === null) {
    context.throwLocalsUndefinedError(name);
    callback(null, null);
  } else {
    getLocalsCase[info[0]](info, name, context, callback);
  }
};
var getLocalsCase = [];
// 静态变量
getLocalsCase[Context.prototype.STATIC_LOCALS] = function (info, name, context, callback) {
...
// 通过函数获取的变量
getLocalsCase[Context.prototype.SYNC_LOCALS] = function (info, name, context, callback) {
...
// 异步获取变量
getLocalsCase[Context.prototype.ASYNC_LOCALS] = function (info, name, context, callback) {
...
execOpcode[OPCODE.COMPILER] = function (ast, context, callback) {
  run(ast[1], context, function (err, val) {
