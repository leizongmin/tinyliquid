...
var getOpArgs2 = function (astList, context, callback) {
  var retval = [];
  retval._c = 0;
  getOpArgs2Item(astList[0], context, retval, 0, callback);
  getOpArgs2Item(astList[1], context, retval, 1, callback);
};
var getOpArgs2Item = function (ast, context, retval, i, callback) {
  if (isAST(ast)) {
    var op = execOpcode[ast[0]];
    if (!op) op = execOpcode[0];
    op(astList, context, );
  } else {
    retval[i] = ast;
    getOpArgs2Callback(retval, callback);
  }
};
var getOpArgs2Callback = function (retval, callback) {
  retval._c++;
  if (retval._c > 1) callback(null, retval);
};
*/
